...
class Fight:
    def __init__(
        self, game: Game, foe: Character, player_started_the_fight: bool
    ):
        self.game = game
        self.foe = foe
    ...
    def display_start_of_fight(self):
        ...
        if self.player_started_the_fight:
            message = (
                f"You attack {enemy_name} (health={self.foe.life_points})."
            )
        else:
    ...
    def foe_attack(self):
        ...
        print(
            f"{self.foe.name} is now hitting you. Dealing {damage} points of damage."
        )

        self.deal_damage(
            attacker=self.foe, defender=self.game.player, damage=damage
        )

    def you_attack(self):
        damage = self.game.player.strength
        print(
            f"You are hitting {self.foe.name} dealing {damage} points of damage."
        )
        self.deal_damage(
            attacker=self.game.player, defender=self.foe, damage=damage
        )
